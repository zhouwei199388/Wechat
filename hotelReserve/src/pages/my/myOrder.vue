<template>
  ...
  <repeat for="{{orders}}">
     <view class="flex-column m-20 bg-white p-20" style="border-radius: 10rpx">
        <view class="flex-row-center border-bottom" style="justify-content: space-between;">
         <view class="font-30 text-content">{{item.hotel}}</view>
         <view class="font-28 text-king">状态</view>
        </view>
       <view class="font-28 text-summary m-t-10">
         房型:{{item.hotelroom}} {{item.roomnumber}}间
       </view>
       <view class="font-28 text-summary">
         入住:{{item.startTime}}     共1晚      离店:{{item.endTime}}
       </view>
       <view class="font-28 text-summary m-b-10">
         地址:深圳市。。。。。布吉大道
       </view>
       <view class="flex-row-center font-30 border-top" style="justify-content: space-between">
         <view class="text-content">金额:258</view>
         <view>支付</view>
       </view>
     </view>
  </repeat>
